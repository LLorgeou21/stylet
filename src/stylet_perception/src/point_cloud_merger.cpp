...
public:
    PointCloudMerger() : rclcpp::Node("point_cloud_merger")
    {
        ...
        sub4_.subscribe(this, "/lidar_4/points");
        sub5_.subscribe(this, "/lidar_5/points");

        sync_ = std::make_shared<message_filters::Synchronizer<SyncPolicy>>(SyncPolicy(10), sub1_, sub2_, sub3_, sub4_, sub5_);
    ...
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg5)
{
    try
    {
        ...
        geometry_msgs::msg::TransformStamped transform4 =
            tf_buffer_->lookupTransform("world", msg4->header.frame_id, msg4->header.stamp);
        sensor_msgs::msg::PointCloud2 transformed4;
        tf2::doTransform(*msg4, transformed4, transform4);

        geometry_msgs::msg::TransformStamped transform5 =
            tf_buffer_->lookupTransform("world", msg5->header.frame_id, msg5->header.stamp);
        ...
        merged.data.insert(merged.data.end(), transformed5.data.begin(), transformed5.data.end());

        merged.height = 1;
        merged.width = transformed1.width * transformed1.height + transformed2.width * transformed2.height +
                        transformed3.width * transformed3.height + transformed4.width * transformed4.height +
                        transformed5.width * transformed5.height;
